...
class KillerZee:
    ...
    def __init__(self, device=0, countrycode="US", rfprofile=RFPROFILE_R2):
        '''
        Instantiates the KillerZee class.

        @type device:   Int
        @param device:  RfCat device index
        @return: None
        @rtype: None
        '''
        self.dev = RfCat(device, debug=False)
    ...
    def inject_r1(self, packet, count=1, delay=1, preamble_len=10):
        '''
        ...
        @type preamble_len: Integer
        @param preamble_len: Desired length of preamble, in bytes, def=10 bytes
        @rtype: None
        '''
        ...
        if preamble_len % 8 == 0: # Even byte length
            PREAMBLE_R2="\x55" * (preamble_len / 8)
            beamdata = PREAMBLE_R2 * 2 + SFD_R2 + "\x55\xFF" # Wake everyone up
            ppdu = PREAMBLE_R2 + SFD_R2 + packet + chr(self.calcfcs(packet)) + EFD_R1
        else: # Uneven byte length
            PREAMBLE_R2="\x55" * ((preamble_len / 8)+1)
            beamdata = PREAMBLE_R2[1:] * 2 + SFD_R2 + "\x55\xFF" # Wake everyone up
            ppdu = PREAMBLE_R2 + SFD_R2 + packet + chr(self.calcfcs(packet)) + EFD_R1
            # Bitwise shift packet left to account for desired preamble length
            # Pad end of packet with 0s
            ppdu_b = (str_to_binary(ppdu) + ("0" * (8 - (preamble_len % 8))))[8 - (preamble_len % 8):]
            ppdu = binary_to_hex(ppdu_b)
    ...
    def inject_r2(self, packet, count=1, delay=1, beam=False, preamble_len=80):
        '''
        Injects the specified packet contents.
        @type packet: String
        @param packet: Packet contents to transmit, without FCS.
        ...
        @param delay: Delay between each frame, def=1.0 seconds
        @type preamble_len: Integer
        @param preamble_len: Desired length of preamble, in bits, def=80 bits
        @rtype: None
        '''

        # To inject, we need to change the sync mode to SYNCM_CARRIER, and change
        # the length to the packet length + preamble + SFD
        # Save the previous sync mode and length settings to restore later
        syncmode = self.dev.getMdmSyncMode()
        plen = self.dev.getPktLEN()[0] # returns a tuple (pktlen, pktctrl0)
        ...
        self.dev.setMdmSyncMode(SYNCM_CARRIER)
        # PktFLEN should be len(ppdu), but changing length is too slow when
        # switching back to receive mode, thus cutting off the response
        # Dirty fix is to set the length to something that is likely
        # to be long enough to fit the response
        self.dev.makePktFLEN(len(ppdu)*2)
        while count != 0:
            if beam:
                for i in range(100):
    ...
    def pcap_next(self, timeout=1000):
        ...
        if self._pktpost_packet == None:
            ...
            # In case preamble did not sync correctly
            # This happens when switching MdmSyncMode too quickly
            # Particularly when switching from SYNCM_CARRIER to SYNCM_CARRIER_15_of_16
            if packet.startswith("\x55\x55") or packet.startswith("\xaa\xaa"):
                # Convert the packet to binary and sync on the SFD
                pkt_b = ''.join(['%08d'%int(bin(ord(i))[2:]) for i in packet])
                sync_word = "0101010111110000"
                try:
                    sync_word_index = pkt_b.index(sync_word) + len(sync_word)
                    pkt_b = pkt_b[sync_word_index:]
                except:
                    pass
                sb = (pkt_b[i:i+8] for i in range(0, len(pkt_b), 8))
                packet = ''.join(chr(int(char, 2)) for char in sb)
            ...
            # Added additional "\x55" to lower the probability of unintended packet loss
            pieces = packet.split("\x55\x55\xf0")
            if len(pieces) > 1:
