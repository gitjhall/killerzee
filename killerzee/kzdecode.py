...
class KillerZeeDecode:
    ...
    LIBRARY_TYPE = {
        "01":"Controller_Static",
        "02":"Controller",
        "03":"Slave_Enhanced",
        "04":"Slave",
        "05":"Installer",
        "06":"Slave_Routing",
        "07":"Controller_Bridge",
        "08":"DUT"
    }
    ...
    def get_manufacturer_specific(self, payload):
        '''
        Decodes ManufacturerSpecific Report payload and returns the name of the manufacturer and product.
        @type src: String
        @param src: Encoded hex string of ManufacturerSpecific Report payload
            (should have length of 12 representing 6 bytes).
        ...
        if payload is not None and len(payload) >= 12:
            ...
            for manufacturer in root.iter(xmlns+'Manufacturer'):
                if manufacturer.get("id") == payload[:4]:
                    manufacturername = manufacturer.get('name')
                    for product in manufacturer:
                        if product.get('type') == payload[4:8] and product.get('id') == payload[8:]:
    ...
    def get_version(self, payload):
        '''
        Decodes Version Report payload and returns the library type, protocol version and subversion, and
            application version and subversion.
        @type src: String
        @param src: Encoded hex string of Version Report payload
            (should have length of 10 representing 5 bytes).
        @rtype: tuple (String, String, String, String, String)
        '''
        librarytype = None
        protoversion = None
        protosubversion = None
        appversion = None
        appsubversion = None
        if payload is not None and len(payload) >= 10:
            if payload[:2] in self.LIBRARY_TYPE:
                librarytype = self.LIBRARY_TYPE[payload[:2]]
            else:
                librarytype = payload[:2]

            # Would like to eventually map these values to actual version numbers...
            protoversion = payload[2:4]
            protosubversion = payload[4:6]
            appversion = payload[6:8]
            appsubversion = payload[8:10]
        return librarytype, protoversion, protosubversion, appversion, appsubversion


    def get_supported_command_classes(self, payload):
        '''
        Decodes Node Information Frame and returns dictionary of supported command classes.
        @type src: String
        @param src: Raw string of NIF payload (should have min length of 7 bytes).
        @rtype: dict {byte (command class value), String (command class name)}
        '''
        cmd_classes = {}
        if payload is not None and len(payload) > 6: # Supported Command Classes are listed after starting with the 7th byte
            for i in range(6, len(payload)):
                if ord(payload[i]) in self.COMMAND_CLASSES:
                    cmd_classes[ord(payload[i])] = self.COMMAND_CLASSES[ord(payload[i])]
                else:
                    cmd_classes[ord(payload[i])] = "Unknown"
        return cmd_classes
